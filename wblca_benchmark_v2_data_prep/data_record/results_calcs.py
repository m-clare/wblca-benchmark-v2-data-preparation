...
def create_tool_specific_columns(
    ...
    lca_full_results_logger.info(
        "Create tool specific columns based on Cat_Ele_2 and Cat_Mat_2."
    )
    output_w_new_cols = (
    ...
    lca_full_results_logger.info(
        "Create NA values based on tool column in lca_results."
    )
    ...
    output_w_new_cols["oneclick_omniclass"] = output_w_new_cols[
        "oneclick_omniclass"
    ].mask(output_w_new_cols["Tool"] == "TallyLCA", "NA")
...
def create_mui(output_w_new_cols: pd.DataFrame) -> pd.DataFrame:
    ...
    output_w_new_cols.loc[reno_mask, "mui_gfa"] = output_w_new_cols[
        "Mass Total (kg)"
    ].div(
        output_w_new_cols["bldg_added_gfa"].astype(float).fillna(0)
        + output_w_new_cols["bldg_renovated_gfa"].astype(float).fillna(0)
    )
    output_w_new_cols.loc[reno_mask, "mui_cfa"] = output_w_new_cols[
        "Mass Total (kg)"
    ].div(
        output_w_new_cols["bldg_added_gfa"].astype(float).fillna(0)
        + output_w_new_cols["bldg_renovated_gfa"].astype(float).fillna(0)
