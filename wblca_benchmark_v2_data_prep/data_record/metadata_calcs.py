...
from wblca_benchmark_v2_data_prep.data_record.output_format import (
    format_buildings_metadata,
)
...
def create_total_impact_columns(
    excluding_d_wblca_output: pd.DataFrame,
...
def create_intensity_columns(project_metadata: pd.DataFrame) -> pd.DataFrame:
    ...
    project_metadata["ec_per_occupant_a_to_c"] = (
        (
            ...
            / project_metadata["bldg_occupants"]
        )
        .astype(float)
        .fillna("NULL")
    )
    project_metadata["ec_per_res_unit_a_to_c"] = (
        (
            project_metadata["Global Warming Potential_Ebio"]
            / project_metadata["bldg_res_units"]
        )
        .astype(float)
        .fillna("NULL")
    )
    ...
    project_metadata.loc[reno_mask, "mui_total_cfa"] = project_metadata[
        "Mass Total (kg)"
    ].div(
        project_metadata["bldg_added_gfa"].astype(float).fillna(0)
        + project_metadata["bldg_renovated_gfa"].astype(float).fillna(0)
    ...
    project_metadata.loc[reno_mask, "mui_total_gfa"] = project_metadata[
        "Mass Total (kg)"
    ].div(
        project_metadata["bldg_added_gfa"].astype(float).fillna(0)
        + project_metadata["bldg_renovated_gfa"].astype(float).fillna(0)
...
def null_override_project_metadata(project_metadata: pd.DataFrame) -> pd.DataFrame:
    ...
    project_metadata.loc[(project_metadata["bldg_sec_use"].isna()), "bldg_sec_use"] = (
        "NA"
    )
    ...
    project_metadata.loc[
        (project_metadata["bldg_park_type"] == "No Parking"), "bldg_park_gfa"
    ] = "NA"
    ...
    project_metadata.loc[
        project_metadata["bldg_res_units"] == "NA", "ec_per_res_unit_a_to_c"
    ] = "NA"
    ...
    project_metadata["eci_a_to_c_gfa"] = project_metadata["eci_a_to_c_gfa"].astype(
        object
    )
    project_metadata.loc[
        project_metadata["bldg_prim_use"] == "Parking", "eci_a_to_c_gfa"
    ] = "NA"
    project_metadata["epi_a_to_c_gfa"] = project_metadata["epi_a_to_c_gfa"].astype(
        object
    )
    project_metadata.loc[
        project_metadata["bldg_prim_use"] == "Parking", "epi_a_to_c_gfa"
    ] = "NA"
    project_metadata["api_a_to_c_gfa"] = project_metadata["api_a_to_c_gfa"].astype(
        object
    )
    project_metadata.loc[
        project_metadata["bldg_prim_use"] == "Parking", "api_a_to_c_gfa"
    ] = "NA"
    project_metadata["sfpi_a_to_c_gfa"] = project_metadata["sfpi_a_to_c_gfa"].astype(
        object
    )
    project_metadata.loc[
        project_metadata["bldg_prim_use"] == "Parking", "sfpi_a_to_c_gfa"
    ] = "NA"
    project_metadata["odpi_a_to_c_gfa"] = project_metadata["odpi_a_to_c_gfa"].astype(
        object
    )
    project_metadata.loc[
        project_metadata["bldg_prim_use"] == "Parking", "odpi_a_to_c_gfa"
    ] = "NA"
    project_metadata["nredi_a_to_c_gfa"] = project_metadata["nredi_a_to_c_gfa"].astype(
        object
    )
    project_metadata.loc[
...
def zero_or_missing_float_loc(project_metadata: pd.DataFrame, column_name: str) -> None:
    ...
    try:
        ...
        project_metadata.loc[(project_metadata[column_name].isna()), column_name] = (
            "NULL"
        )
    except ValueError as ve:
        metadata_logger.exception(
            "loc function for zero or missing floats cannot be assigned"
        )
        raise ValueError(
            "loc function for zero or missing floats cannot be assigned"
        ) from ve
...
def write_buildings_metadata_to_excel(
    ...
    with pd.ExcelWriter(
        ...
        metadata_logger.info(
            "Buildings_metadata has beeen saved to %s", public_dataset_directory
        )
