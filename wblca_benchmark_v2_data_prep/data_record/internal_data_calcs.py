...
def create_internal_data(accepted_design_phases: list,
                         dets: pd.DataFrame) -> pd.DataFrame:
    ...
    internal_data = internal_data.loc[
        internal_data['final_report'] == 'Yes'
    ].loc[
        internal_data['design_phase'].isin(accepted_design_phases)
...
def clean_internal_data(internal_data: pd.DataFrame,
                        column_value_replace: dict,
                        public_dataset_column_renaming: dict,
                        metadata_col_order: list) -> pd.DataFrame:
    ...
    # rename columns
    internal_data_logger.info('Reename columns based on public_dataset_column_renaming.')
...
def write_internal_data_to_excel(internal_data: pd.DataFrame,
    ...
    with pd.ExcelWriter(
        ...
        internal_data_logger.info("Internal data has beeen saved to %s", public_dataset_directory)
