...
def create_all_oneclick_filters(df: pd.DataFrame) -> Dict[str, pd.Series]:
    ...
    filters = {
        ...
        'oc_omni_shell_enc': df['Omniclass'].str.match(
            '21-02 2|21-02 3'
        ),
        ...
        'oc_n_glass_sheath': df['Name'].str.contains(
            'glass mat sheathing'
        ),
        'oc_n_carpet': df['Name'].str.contains(
            'carpet|Carpet|CARPET'
        ),
        'oc_n_cladding': df['Name'].str.contains(
            'cladding|Cladding|CLADDING'
        ),
        'oc_n_flooring': df['Name'].str.contains(
            'flooring|Flooring|FLOORING'
        ),
        'oc_n_ceil_pan': df['Name'].str.contains(
            'Ceiling Panels'
        ),
        'oc_n_acoustic': df['Name'].str.contains(
            'acoustic|Acoustic|ACOUSTIC'
        ),
        'oc_n_deck': df['Name'].str.contains(
            'deck|Deck|DECK'
        ),
        'oc_n_timber': df['Name'].str.contains(
...
def create_all_tally_filters(df: pd.DataFrame) -> Dict[str, pd.Series]:
    ...
    filters = {
        ...
        'rt_fn_spandrel': df['Revit family name'].str.contains('spandrel|Spandrel|SPANDREL'),
        'rt_fn_partition': df['Revit family name'].str.contains('partition|Partition|PARTITION'),
        'rt_fn_bsmnt': df['Revit family name'].str.contains('basement|Basement|BASEMENT'),
        'rt_fn_retain': df['Revit family name'].str.contains('retaining|Retaining|RETAINING'),
        'rt_fn_stem': df['Revit family name'].str.contains('stem|Stem|STEM'),
        'rt_fn_cistern': df['Revit family name'].str.contains('cistern|Cistern|CISTERN'),
        'rt_fn_site': df['Revit family name'].str.contains('site|Site|SITE'),
        'rt_fn_battered': df['Revit family name'].str.contains('battered|Battered|BATTERED'),
        'rt_fn_caisson': df['Revit family name'].str.contains('caisson|Caisson|CAISSON'),
        ...
        'rt_fn_topping': df['Revit family name'].str.contains('topping|Topping|TOPPING'),
        ...
        'rt_fn_p_naming': df['Revit family name'].str.match('P-|P1|P2|P3|P4|P5|P6|P7|P8|P9'),
        'rt_fn_wall_w': df['Revit family name'].str.match(
            'W-|\\(W|\\(W-|W\\[|W[0-9]'
        ),

        'ty_ed_09': df['Tally Entry Division'].str.fullmatch('09 - Finishes'),
        'ty_ed_08': df['Tally Entry Division'].str.fullmatch('08 - Openings and Glazing'),
        'ty_ed_07': df['Tally Entry Division'].str.fullmatch(
            '07 - Thermal and Moisture Protection'
        ),
        'ty_ed_06': df['Tally Entry Division'].str.fullmatch('06 - Wood/Plastics/Composites'),
        ...
        'ty_en_ply_int': df['Tally Entry Name'].str.fullmatch('Plywood, interior grade'),
        'ty_en_mdf': df['Tally Entry Name'].str.fullmatch('Medium density fiberboard \\(MDF\\)'),
        'ty_en_ply_ext': df['Tally Entry Name'].str.fullmatch('Plywood, exterior grade'),
        'ty_en_ply_lvl': df['Tally Entry Name'].str.fullmatch('Laminated veneer lumber \\(LVL\\)'),
        'ty_en_ply_osb': df['Tally Entry Name'].str.fullmatch('Oriented strandboard \\(OSB\\)'),
        ...
        'ty_en_fib_ins': df['Tally Entry Name'].str.fullmatch('Fiberglass mat gypsum sheathing'),
