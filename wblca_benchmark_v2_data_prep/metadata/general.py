...
def write_to_csv(df: pd.DataFrame, write_directory: Path,
                 file_suffix: str, module_description: str) -> str:
    ...
    try:
        df.to_csv(
            write_directory.joinpath(
                f'{df.attrs.get("name")}{file_suffix}'
            )
        )
    except PermissionError as pe:
        general_logger.exception('Permission Error probably caused by having file open')
    ...
    general_logger.info("data entry template for firm %s has \
been saved in the %s folder", df.attrs.get('name'), module_description)
