...
def read_excel(file_path: Path, sheet_name: str) -> pd.DataFrame:
    ...
    try:
        ...
        organize_logger.exception('Permission Error probably caused by having file open')
    ...
    organize_logger.info('Read data from sheet %s from file %s', sheet_name, file_path.name)
    try:
        df.attrs = {
...
def replace_columns(df: pd.DataFrame, replacements_yaml: dict,
                    replacement_list_name: str) -> pd.DataFrame:
    ...
    if replacements_yaml.get(replacement_list_name):
        df['Field'] = df['Field'].replace(
            replacements_yaml.get(replacement_list_name)
        )
...
def column_name_test(df: pd.DataFrame, original_col_list: list) -> None:
    ...
    try:
        ...
        for error in str(e).split('\n')[6:]:
            organize_logger.error("Column name issue for firm %s: %s", df.iloc[0, 3], error)
...
def column_name_after_transpose_test(df: pd.DataFrame, original_col_list: list) -> None:
    ...
    if len(index_difference) == 0:
        organize_logger.info('There were no issues with column names after \
transpose for firm %s', df.attrs.get("name"))
    for column in index_difference:
        organize_logger.error(
            "Column Name issue after transpose for firm %s: %s", df.attrs.get('name'), column
        )
