...
def create_buildings_metadata():
    ...
    internal_data = pd.read_excel(internal_data_path, index_col=False).set_index(
        "project_index"
    )
    ...
    buildings_metadata_logger.info(
        "Exclude Module D and scopes that are not in %s.", scope_type
    )
