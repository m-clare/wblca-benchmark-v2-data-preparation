...
def finalize_det():
    ...
    # instantiate logger
    setup_logger(
        log_file_path=main_directory.joinpath("data/logs/metadata/finalize_det.log"),
        level="info",
    )
    ...
    assert column_removal_list is not None, (
...
the column removal list"
    ...
    assert column_rename_dict is not None, (
        "The dict for column renaming could not be set"
    )
    column_finalize_dict = col_finalize.get("column_finalizing")
    assert column_finalize_dict is not None, (
        "The dict for column finalizing could not be set"
    )
    ...
    utils.write_to_csv(
        combined_det, write_data_directory_path, file_suffix, "finalized"
    )
