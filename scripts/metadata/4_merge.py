...
def merge_det():
    ...
    # instantiate logger
    setup_logger(
        log_file_path=main_directory.joinpath("data/logs/metadata/merge_det.log"),
        level="info",
    )
    ...
    read_data_directory = config.get(current_file_path.stem).get("read_data_directory")
    read_data_filter = config.get(current_file_path.stem).get("read_data_filter")
    write_data_directory = config.get(current_file_path.stem).get(
        "write_data_directory"
    )
    module_description = config.get(current_file_path.stem).get("module_description")
    merged_file_suffix = config.get(current_file_path.stem).get("merged_file_suffix")

    # set paths
    read_data_directory_path = main_directory.joinpath(read_data_directory).glob(
        read_data_filter
    )
    read_data_directory_for_filtering = main_directory.joinpath(read_data_directory)
    ...
    for firm in firm_names:
        ...
        project_det = list(read_data_directory_for_filtering.glob(f"*{firm}_project*"))[
            0
        ]

        e_det_df = m_utils.read_energy_csv(energy_det)

        p_det_df = utils.read_csv(project_det)

        try:
            merged_det = p_det_df.join(e_det_df)
            merged_det.attrs = {"name": firm}
