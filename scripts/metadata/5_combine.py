...
def combine_det():
    ...
    setup_logger(
        log_file_path=main_directory.joinpath(
            'data/logs/metadata/combine_det.log'
        ),
        level='info'
    )
    ...
    for file in read_data_directory_path:
        df = co_utils.read_merged_csv(
            file,
            col_dtypes.get('parse_dates')
        )
        df_list.append(df)
    main_combine_logger.info('Read all merged data entry templates.')

    main_combine_logger.info('Begin concatenation process of all merged data entry templates.')
    try:
    ...
    energy_schema = get_energy_det_schema().remove_columns(
